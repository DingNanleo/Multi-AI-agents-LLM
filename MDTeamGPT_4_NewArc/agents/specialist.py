...
###### specialist0.py featue ########
###### let all specialist speak round by round ########

class SpecialistAgent(BaseAgent):
    def __init__(self, specialist_info: Dict, question: str, options: List[str]):
        super().__init__(
...
class SpecialistDoctor:
    def __init__(self, historical_pool=None):
        self.specialists = []
...
#============================ Main method to handle specialist consultation process ======================   
    def perform_task(self, primary_care_response: Dict, question: str, options: List[str]) -> Dict:
        try:
            ...
            self.specialists = [
                SpecialistAgent(info, question, options)
    ...
    def _start_discussion(self, max_rounds: int = 100) -> Dict:
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            if round_num == 1:
                ...
                # Discussion rounds
                current_opinions = self._conduct_discussion_round(current_opinions, round_num)
...
#=====================================================================================================
    ...
    def provide_initial_opinion(self, specialist: SpecialistAgent) -> Dict:
        ...
        while retry_count < max_retries:
            ...
            print(f"\n====== {specialist.specialist_type} Attempt {retry_count+1} ======")
    ...
    #     prompt_base = f"""
    #     You are a {specialist.specialist_type} with the following expertise: {specialist.role_description}.
    ...
    #     You MUST respond in this EXACT format, example of VALID response::
    #     {{
    #     "selected_option": {{
    #         "key": "A", 
    #         "value": "Cardiac Tamponade"
    #     }},   // MUST match one of: {specialist.options}
    #     "reasoning": "The patient's JVD suggests..."
    #     }}
    #     """
    #     max_retries = 5  # the try to get right format response, prevent infinite loops
    #     retry_count = 0

    #     while retry_count < max_retries:
    #         prompt = prompt_base
    #         if retry_count > 0:
    #             prompt += "\n\nYour previous response did not meet the required format. Please try again and ensure your response matches the exact JSON format specified, with all required fields."
            
    #         print(f"\n====== {specialist.specialist_type} Round 1 Attempt {retry_count+1} Prompt ======")
    #         print(prompt)

    #         print(f"\n====== {specialist.specialist_type} Round 1 Attempt {retry_count+1} LLM Response ======")
    #         llm_response = specialist.call_llm(prompt,require_json=True)
    #         print(llm_response)

    #         parsed_response = self._parse_and_validate_initial_response(llm_response, specialist.specialist_type, specialist.options)

    #         if parsed_response is not None:
    #             print(f"\n====== {specialist.specialist_type} Parsed Response ======")
    ...
    #         "specialist": specialist.specialist_type,
    #         "selected_option": specialist.options[0],
    #         "reasoning": "Default selection after failing to get valid response format"
    #     }
...
#=====================================================================================================
    ...
    def _conduct_discussion_round(self, previous_opinions: List[Dict], round_num: int) -> List[Dict]:
        #print(f"\nPrevious_opinions (Round {round_num}):\n{previous_opinions}\n")
        discussion_summary = self._create_discussion_summary(previous_opinions,round_num)
        #print(f"\nDiscussion Summary (Round {round_num}):\n{discussion_summary}\n")
        
        updated_opinions = []
        for specialist, prev_opinion in zip(self.specialists, previous_opinions):
            updated_opinion = self._get_updated_opinion(
                specialist, 
                discussion_summary,
                prev_opinion,
                round_num
    ...
    def _get_updated_opinion(self, specialist, discussion_summary, prev_opinion, round_num) -> Dict:
        """Get an updated opinion from a specialist after discussion with clear defense/adjust logic"""
        prompt = f"""
        [IMPORTANT CONTEXT]
        ...
        [DISCUSSION SUMMARY - ROUND {round_num-1}]
        {discussion_summary}
        ...
        {{
            ...
            "reasoning": {{
                /// REQUIRED for DEFEND
                "your_position_strengths": "1-2 specific advantages of your position", // Only for DEFEND
                "others_weaknesses": "1-2 flaws in other positions",  // Only for DEFEND
                // REQUIRED for ADJUST
                "old_position_weaknesses": "1-2 weaknesses in original stance",  // Only for ADJUST
                "new_insights": "1-2 points that changed your view"  // Only for ADJUST
            }},
            "round": {round_num},
            "specialist": "{specialist.specialist_type}"
        }}
    ...
    def _parse_and_validate_response(self, llm_response: Union[str, dict], specialist_type: str, 
        ...
        """
        Combined validation and parsing in one step.
        Returns None if invalid, formatted response dict if valid.
        """
        # --- JSON Validation ---
        if isinstance(llm_response, str):
            try:
