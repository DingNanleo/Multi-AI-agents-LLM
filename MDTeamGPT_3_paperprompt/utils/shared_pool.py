...
class HistoricalSharedPool:
    ...
    def add_statements(self, statements: Dict):
        ...
        #if "timestamp" not in statements:
            #statements["timestamp"] = datetime.now().isoformat()
            
        # Merge the new statements with existing pool
        ...
        print(f"Stored statements: {list(statements.keys())}")
        self.save_to_file()
    ...
    def get_specialist_opinions_by_round(self, round_num: int) -> List[Dict]:
        """
        Get specialist opinions for a specific round
        Args:
            round_num: The round number to get opinions from
        Returns:
            List of specialist opinions for the specified round
        """
