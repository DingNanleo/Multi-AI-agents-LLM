...
#------------------------Step 1: Primary care assigns specialists-----------------------
    ...
    while True:
        ...
        # for i, spec in enumerate(selected_specialists, 1):
        #     try:
        #         print(f"[DEBUG] Specialist {i}:")
        #         print(f"  - Name: {spec.name}")
        #         print(f"  - Role: {spec.role}")
        #         print(f"  - Specialty: {spec.specialty}")
        #         print(f"  - Config: {spec.specialty_config}")
        #     except AttributeError as ae:
        #         print(f"  - ERROR accessing attributes: {str(ae)}")
...
#------------------------Step 2: Specialists provide opinions------------------------
    print("="*80)
    print(f"=====Step 2: Specialists Agent=====")
    max_rounds = 5
    final_opinion = None
...
#------------------------Step 4: Check Consensus------------------------
        ...
        # # Method 2: Similarity-based consensus (handles minor differences like pluralization)
        # else:
        #     # Calculate pairwise similarity between all choices
        #     similarity_scores = []
        #     for i in range(len(all_choices)):
        #         for j in range(i+1, len(all_choices)):
        #             similarity_scores.append(similar(all_choices[i], all_choices[j]))
            
        #     if similarity_scores:  # Only proceed if we have comparisons
        #         avg_similarity = sum(similarity_scores) / len(similarity_scores)
        #         print(f"Average similarity between choices: {avg_similarity:.2f}")
                
        #         # If average similarity is above threshold (e.g., 0.9), consider it consensus
        #         if avg_similarity > 0.95 and not any("Error:" in choice for choice in all_choices):
        #             # Take the most common choice among similar options
        #             from collections import Counter
        #             choice_counts = Counter(all_choices)
        #             most_common = choice_counts.most_common(1)[0][0]
        #             final_opinion = most_common
        #             print(f"Similarity-based consensus reached: {most_common}")
        #             break
        ...
        if round_num == max_rounds:
            print("  >>>>>>>>>> Final round reached - must determining majority choice >>>>>>>>>>>>")
...
#------------------------Step 4: Safety and ethics review------------------------
    ...
    reformatted_ethics_opinion = {
        "Safety_Ethics_Opinion": {
            ...
            "Specialists_Answer": safety_ethics_opinion.get("Specialists Answer", ""),
            "Ethics_Answer": safety_ethics_opinion.get("Ethics Answer", ""),
            "Ethics_Conclusion": safety_ethics_opinion.get("Ethics Conclusion", "")
        }
    }
...
#------------------------Step 5: Chain of thought reviewer------------------------
    ...
    #print("\n===Chain-of-thought processing result==== ") 
    #print(f"\n{review_result}") 
