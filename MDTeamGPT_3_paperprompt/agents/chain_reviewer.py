...
#------------------------Base part prompt--------------------------
        right_parts = [
            "ROLE DESCRIPTION:",
            "You are a Chain-of-Thought Reviewer, responsible for extracting detailed chains of thought from each doctor’s diagnostic process. ",
            "Your task is to analyze and record the doctors’ thought processes (information from the Historical Shared Pool) and store this information in a vector database for future analysis and retrieval.",
            "You will process experience based on whether the consultation outcome is correct or incorrect,storing the relevant information for both correct and incorrect answers.",
            "",
            "TASK: ",
            "Step 1: Identify the Chain of Thought",
            ...
            "Step 2: Structuring the Chain of Thought",
            ...
            "Step 3: Store the Correct Answer",
            ...
            # "The content should include Question (which is the medical problem), Answer(which is specialist final answer), and Summary(summary the answer in Step 2: Structuring the Chain of Thought).",
        ...
        wrong_parts = [
            ...
            "Step 4: Document the Reflection Process",
            ...
            "Step 5: Store the Incorrect Answer",
            ...
            # "Content to store are: Question (which is the medical problem), Correct Answer(the official answer of the question), initial hypothesis, analysis process, final Conclusion(which is specialist final answer),and error reflection). ",
            # "For incorrect diagnoses, all rounds from the Historical Shared Pool should be used for observation to analyze how the reasoning evolved over time. "
            ...
            "     }",
            "" 
            # "This structure will include all rounds of reasoning from the Historical Shared Pool, with each round contributing valuable information for the error reflection and analysis."
            # "",
            ]
    ...
    def _history_retrieve(self,history: Dict,is_correct:str) -> str:
        ...
        for key in metadata.keys():
            if isinstance(key, str) and key.startswith('round '):
                try:
                    ...
                    #print(f"round_numbers: {round_numbers}")
        ...
        if is_correct:
            ...
            history_txt += "\nSpecialist Opinions: \n"
            for opinion in specialist_opinions:
                agent = opinion.get('Agent_Name', 'Unknown Specialist')
                reasoning = opinion.get('Reasoning', 'No reasoning provided')
                choice = opinion.get('Choice', 'No choice provided')

                history_txt += f"{agent}:\n"
                history_txt += f"- Reasoning: {reasoning}\n"
                history_txt += f"- Conclusion: {choice}\n\n"

            # Add lead physician analysis
            if lead_physician:
                history_txt += "\nLead Physician Analysis: \n"
                consistency = lead_physician.get('consistency', [])
                conflicts = lead_physician.get('conflict', [])  
                independence = lead_physician.get('independence', [])
                integration = lead_physician.get('integration', [])
                #print(f"\n consistency:{consistency}")

                if consistency:
                    history_txt += f"Consistency: {', '.join(consistency)}\n"
                if conflicts:
                    history_txt += f"Conflict: {'; '.join(conflicts)}\n"
                if independence:
                    history_txt += f"Independence: {', '.join(independence)}\n" 
                if integration:
                    history_txt += f"Integration: {'; '.join(integration)}\n"
            #print(f"history txt: {history_txt}\n")
    ...
    def _store_correct_answer(self, data: Dict) -> None:
        ...
        clean_data = {
            ...
            "Correct Answer": data.get("Answer"),
